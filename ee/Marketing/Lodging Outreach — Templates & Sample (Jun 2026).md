# Lodging Outreach — Templates & Sample (Jun 2026)

Status: DRAFT for David's approval. Nothing sent yet. Sample-first rule: David approves the sample before any batch goes out.

Approach: warm intro first, no ask, no offer in the opener. The 2-pass comp offer is extended only on reply (template #5). Send from davey@ebbandember.com, cc media@ebbandember.com. Sign "Davey, Ebb & Ember." Scrub against Do Not Contact List before sending. A/B the two subject lines per batch. For properties with no usable email, paste the matching body into their website contact form; for platform-only houseboat/Airbnb listings, message the host through the listing.

Voice: short, skimmable, concrete, no wellness buzzwords, no em dashes, max one exclamation.

**Confirmed facts (use exactly, never improvise):**
- Location: **at Elevated Tides on the Columbia** (173 NE Bridgeton Rd, Portland OR). Not "Bridgeton marina."
- We are **Portland's only floating sauna**.
- The sauna is **gas-fired**, but leave the fuel unsaid in copy. Never say wood-fired.
- Tone: **neighbor framing for hyperlocal properties only** (Hayden Island / Jantzen Beach / Bridgeton / Vancouver-close / airport corridor / nearby STR + houseboats). Farther-metro and downtown properties use the non-neighbor opener (template 2).
- Fill `[X minutes]` only with a real, verified drive time. Do not guess a number.

**Attach + socials on every first-touch send:**
- Attach the hero photo: `Assets/Photos/2026-06-23 Sunny Session/web/ebb-ember-floating-sauna-covered-deck-kayaks-columbia-river.jpg` (covered cedar deck framing the sauna across the water with teal kayaks; cleared to use, ~461KB). For contact-form submissions that don't allow attachments, paste a link to the photo or the Instagram instead.
- Signature lists all socials as hyperlinked words in the HTML send (recipient sees clean text, real URLs behind them):
  - Instagram = https://instagram.com/ebbandember (clean, confirmed)
  - Facebook = https://facebook.com/633573899839067 (page "Ebb & Ember Floating Saunas"; numeric URL works, no vanity confirmed)
  - YouTube = https://youtube.com/channel/UCbzG-G9b9s9Uz49QZuGqOeQ (channel-ID URL works; YT auth currently down, swap to the @handle once known)
  - Swap in cleaner @handles/vanity URLs if David provides them. Do NOT invent handles.
  - For plain-text contact forms (no hyperlinks): write "Instagram: instagram.com/ebbandember, plus Ebb & Ember Floating Saunas on Facebook and YouTube."

---

## 1. Hyperlocal hotel / concierge (neighbor tone). Sample filled below.

Subject A: A local spot your guests keep asking us about
Subject B: Something only-in-Portland near you

Hey [name],

Friendly note from a neighbor. We run Ebb & Ember, Portland's only floating sauna, out on the Columbia at Elevated Tides, about [X minutes] from you. Real heat out on the water, with a cold plunge straight into the river if guests want it.

Your front desk probably gets the "what's something we can't do anywhere else" question a lot. We tend to be a good answer to it, especially for folks who've already done the usual Portland list.

No ask here. Just wanted to be on your radar as a recommendation for guests. If your team is ever curious to come see it for themselves, reply and we'll set it up.

Davey, Ebb & Ember
ebbandember.com
Follow along: Instagram | Facebook | YouTube

---

### Sample (filled, for approval) — Oxford Suites Jantzen Beach

Subject A: A local spot your guests keep asking us about

Hey [front desk lead],

Friendly note from a neighbor. We run Ebb & Ember, Portland's only floating sauna, out on the Columbia at Elevated Tides, about [X minutes] from you. Real heat out on the water, with a cold plunge straight into the river if guests want it.

Your front desk probably gets the "what's something we can't do anywhere else" question a lot. We tend to be a good answer to it, especially for folks who've already done the usual Portland list.

No ask here. Just wanted to be on your radar as a recommendation for guests. If your team is ever curious to come see it for themselves, reply and we'll set it up.

Davey, Ebb & Ember
ebbandember.com
Follow along: Instagram | Facebook | YouTube

(Note: fill the [X minutes] drive time from Oxford Suites once verified, do not guess it.)

---

## 2. Farther-metro / downtown / boutique hotel (no neighbor tone)

Subject A: A Nordic-style sauna on the Columbia for your guests
Subject B: For the guests who come to Portland to slow down

Hey [name],

We're Ebb & Ember, Portland's only floating sauna, out on the Columbia at Elevated Tides. Real heat out on the water, a cold plunge into the river, the kind of slow couple of hours your guests don't get many other places in town.

Felt like a fit with the crowd you host, so wanted to put us on your radar as a recommendation. No ask. If anyone on your team wants to come experience it before you point guests our way, just reply and we'll get you out on the water.

Davey, Ebb & Ember
ebbandember.com
Follow along: Instagram | Facebook | YouTube

---

## 3. Airbnb / STR / houseboat host (neighbor tone, guest-book angle)

Subject A: A neighbor recommendation for your guests
Subject B: Something fun to add to your guest book

Hey [name],

Quick hello from down the way. We run Ebb & Ember, Portland's only floating sauna, out on the Columbia at Elevated Tides. Real heat out on the water with a cold plunge into the river, open 7am to 9pm daily.

A new way to connect your guests to the river, and an easy rec when they ask what's worth doing in Portland. If you ever want to come try it yourself so you can recommend it for real, reply and we'll set you up.

Davey, Ebb & Ember
ebbandember.com
Follow along: Instagram | Facebook | YouTube

(For a metro-wide STR manager who isn't really local, drop "from down the way" and use template 2's opener instead.)

---

## 4. Follow-up bump (4-5 days later, no reply)

Subject: re: [original subject]

Hey [name],

Just floating this back up in case it slipped by. Still happy to have your team come see the sauna anytime. Reply whenever works.

Davey, Ebb & Ember

---

## 5. Reply / warm-response — THE 2-PASS OFFER LIVES HERE

(Send when they reply with any interest. This is where the comp passes come in.)

Hey [name],

Glad you're up for it. Here are two passes on us, good through August 31, so you or your team can come experience a session and see what we're about. If you're slammed and can't get out, they're yours to gift to a guest or a friend, totally fine either way.

[CODE] gets you two social sessions at ebbandember.com/book. Pick whatever time works, we're open 7am to 9pm daily. Bring a swimsuit and a towel, we handle the rest.

Once you've been out, if it feels like a fit for your guests, I'd love to talk about an easy way to point them our way. No pressure on that, the visit comes first.

Davey, Ebb & Ember

---

## The support menu (later, once they've visited — pick what's easy for them)

The goal is broader than a backlink. After a partner has experienced it, offer a low-friction menu and let them choose whatever fits how they operate. Never ask for all of it at once. Lead with whatever is easiest for that property type.

- **Front desk / staff recommendation** — the big one. Staff who've done it sell it for real. The comp passes exist to make this happen.
- **Guest newsletter mention** — "If you send a guest newsletter, a line about us would mean a lot. I can send a short blurb and a photo."
- **In-room / guest-book / things-to-do page** — add us to their guest guide. If it's a web page, that's also the backlink: hand them a clean URL + suggested anchor.
- **Blog post / local feature** — for properties that publish a blog or local guide, offer to host their team for a session and supply photos so they can write it up.
- **Social mention / share** — "If you ever repost local spots on Instagram, tag @ebbandember and we'll share you right back."
- **Coverage** — if they have local press relationships or a marketing person, a joint angle (a Portland hotel + Portland's only floating sauna) is an easy pitch.
- **Reciprocal, always:** newsletter mention + social support via Kimberlynn. Make it a two-way street.

Until the "things to do near Hayden Island" page exists, point any links at ebbandember.com/experiences or the homepage. Blog isn't live yet, so don't promise a blog link we can't deliver.
