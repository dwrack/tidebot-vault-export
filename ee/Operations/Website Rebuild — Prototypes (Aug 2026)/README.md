...
- **Elevated Tides v2 (2026-08-27):** added the metaphor layer per Davey: geese flocks and a gliding heron crossing the WebGL sunset; a "Wood on water" canvas strip (drifting cedar planks, bobbing ducks, a sea lion that surfaces occasionally) with the timber-heritage copy ("a sauna is just wood on water that learned to keep its heat"); a scroll-driven "A marina, surfacing" SVG scene where the dock, both saunas, masts, and a heron rise out of the waterline with reflections; and a sea lion that periodically surfaces in the tide divider. All animations respect prefers-reduced-motion.
- Verified working in-browser (video playback, live USGS fetch, all canvas/SVG animation). Earlier "video won't decode" scare was just Chrome background-tab throttling; foregrounding the tab confirmed everything plays.
