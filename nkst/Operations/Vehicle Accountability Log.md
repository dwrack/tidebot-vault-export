# Vehicle Accountability Log

**Company:** New Orleans Kayak Swamp Tours / Nola Kayak Tours
**Last Updated:** April 2026

> This log tracks vehicle use, fuel, cleaning, and condition after every tour. Guides confirm via a 3-question post-tour text (sent automatically by the CRM). Manager reviews weekly. Mileage is tracked here for oil change scheduling (every 3,500 miles).

---

## How It Works

**Guides:** After every tour, the CRM sends an automatic text asking three questions. Reply with yes/no. Takes 60 seconds. That's it.

**Manager:** Reviews the log weekly. Flags any "no" answers, mileage milestones, or recurring issues. Follows up with the guide if anything is outstanding.

**Claude:** Summarizes the weekly log and flags: oil change due date, any guides with unresolved "no" items, mileage trend, and any equipment issues that appeared more than once.

---

## Post-Tour Guide Text (Sent by the CRM Automatically)

> "Hey [Name] -- quick wrap-up for today's tour. Three questions:
> 1. Van back at Piety St, locked, no valuables left inside?
> 2. Gas topped off at Shell (code 6006)?
> 3. PFDs hung to dry, MiraZyme sprayed, gear stored?
>
> Reply 1/2/3 as yes or no. Anything to flag? Drop it here."

Any "no" or freeform note routes to the manager's CRM inbox immediately.

---

## Weekly Summary Format

Claude generates this from the weekly log entries. Manager reviews Monday morning.

| Item | This Week |
|---|---|
| Tours run | |
| Van 1 total miles driven | |
| Van 1 current odometer (est.) | |
| Oil change due in approx. | |
| Times gas was confirmed topped off | / total tours |
| Times van was confirmed cleaned | / total tours |
| Times PFDs confirmed done | / total tours |
| Outstanding "no" items | |
| Equipment issues flagged | |

---

## Running Tour Log

*Add one row per tour. Guide confirms via text -- manager or Claude logs the responses.*

| Date | Guide | Tour | Van | Miles Out | Miles In | Miles Driven | Gas Topped | Van Cleaned | PFDs Done | Issues Flagged |
|---|---|---|---|---|---|---|---|---|---|---|
| | | | | | | | | | | |
| | | | | | | | | | | |
| | | | | | | | | | | |

---

## Oil Change Tracker

**Interval:** Every 3,500 miles
**Shell card code:** 6006 (gas only -- not for maintenance purchases)
**Maintenance expenses:** Submit receipt to owner for reimbursement

| Van | Last Oil Change Date | Odometer at Last Change | Current Odometer (est.) | Miles Until Next Change | Due Date (est.) |
|---|---|---|---|---|---|
| Van 1 | | | | | |
| Van 2 (if applicable) | | | | | |

---

## Tire Pressure Reference

| Position | Target PSI |
|---|---|
| Front tires | 60 psi |
| Rear tires | 75 psi |

Guides should check tire pressure on tour days before departure. Flag any low readings in the post-tour text.

---

## Open Issues Log

*Anything flagged by a guide that hasn't been resolved yet. Clear items when fixed.*

| Date Flagged | Guide | Issue | Status | Resolved |
|---|---|---|---|---|
| | | | | |

---

## Inventory Reference (Confirm Before Each Tour Departure)

These items live in the van. Guides confirm before leaving Piety St. Report missing items in the post-tour text.

- [ ] PFDs -- minimum 15 on board
- [ ] Paddles -- count and inspect for cracks
- [ ] First aid kit
- [ ] Guide bag
- [ ] Tow rope
- [ ] Merchandise box: koozies, phone cases, stickers, t-shirts
- [ ] Water and granola bars
- [ ] Gas card on keychain (Shell, code 6006)
- [ ] Clipboard with condition sheet
- [ ] Trailer wheels unlocked before departure

---

*Related: [[Operations/Vehicle Checklists]] · [[SOPs/New Orleans Kayak Swamp Tours — SOP Pre-Tour Preparation]]*
