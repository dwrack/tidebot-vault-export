# Post-Tour Vehicle & Equipment Damage Report
*Guides: fill any time you notice damage or a problem. Routes to contractor.*

Use this any time you see damage, hear a new noise, find a broken strap, scrape a kayak, lose a paddle, drop a phone in the bayou, or anything else that wasn't there when you started. We'd rather know small things early than find out from a customer.

This is not a punishment form. Flagging damage is exactly what we want from you. The only wrong move is hiding it.

---

## Submission

Fill this out at the end of the tour day, before you leave the staging location. Submit by:
- Slack: post in the #ops channel with this template filled in, OR
- Text: send to contractor with photos

Contractor logs it in the Vehicle Maintenance Log within 24 hours and decides on next action.

---

## Template

**Date:** _______________
**Guide:** _______________
**Tour:** _______________
**Vehicle (if applicable):** _______________
**Mileage at end of day (if vehicle):** _______________

**What was damaged or noticed:**
_______________________________________________

**When did it happen?**
- [ ] Before tour (existed when I started)
- [ ] During tour (specify when)
- [ ] After tour (loading / unloading)
- [ ] Not sure

**Did it affect the tour or guests?**
- [ ] No
- [ ] Yes (describe): _______________

**Photos attached?** (Yes / No, attach in Slack or text)

**Severity self-assessment:**
- [ ] Minor (cosmetic, functional)
- [ ] Needs attention before next use
- [ ] Cannot be used until repaired

**Any guest involvement?** (e.g., guest dropped paddle, guest scratched van loading, guest claim of injury)
_______________________________________________

**Anything else contractor should know?**
_______________________________________________

---

## Common items to flag

**Vehicle:**
- New noise (rattle, squeak, grinding, anything you didn't hear yesterday)
- Warning light on dash
- Soft brake pedal
- Pull to one side
- Tire low or worn
- Scrape, dent, broken mirror
- Wiper streaking or not working
- AC or heat not working
- Fuel gauge or other gauge wrong

**Kayaks:**
- Hull crack, scratch deeper than gel coat
- Skeg / rudder damage
- Seat broken or strap torn
- Hatch cover lost or won't seal
- Bungee snapped

**Paddles:**
- Crack in shaft
- Blade chip
- Drip ring missing
- Lost overboard

**PFDs:**
- Strap torn
- Buckle broken
- Foam compressed flat
- Visible mildew

**Other gear:**
- Whistle missing from PFD
- Dry bag leaking
- First aid kit used (note what was used so it can be restocked)
- Bilge pump missing
- Throw rope frayed

---

## What happens next

1. Contractor logs in Vehicle Maintenance Log (or equipment log)
2. Contractor assesses repair vs replace
3. If a customer was involved, contractor opens an Incident Report
4. Resolution and date logged
5. Guide gets a quick reply confirming receipt; nothing more required from guide unless follow-up info needed
