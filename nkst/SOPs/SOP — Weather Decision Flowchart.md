...
## Hard Cancel Thresholds (no tour, no exceptions)

Any one of these triggers an automatic cancel. Not a delay, not a "wait and see." Cancel.

| Condition | Threshold | Source |
|-----------|-----------|--------|
| Sustained wind | 15+ mph | NOAA forecast, on-site anemometer, or visible whitecaps in the bayou |
| Lightning | Any strike within 10 miles | Weather radar, MyRadar app, NOAA |
| Visibility | Less than 0.25 mile (heavy fog, heavy rain) | Visual at the launch |
| Water temp | Below 50°F | NOAA water temp or thermometer at launch |
| Tornado / severe storm warning | Anywhere in tour parish | NOAA |
| Flash flood warning | At launch site or downstream | NOAA |
| Hurricane evacuation order | Any level | Parish emergency management |

If any of these is true at tour start, no tour. If any becomes true mid-tour, return to launch immediately by safest route.
...
```
TOUR DAY MORNING (3 hours before launch)
│
├─ Check NOAA forecast for tour location and tour time window
├─ Check radar
├─ Check tide chart (Honey Island, Bayou Bienvenue, Manchac as applicable)
│
...
│
LAUNCH SITE (at the ramp, before guests are on water)
│
├─→ Visual check: wind on water, sky, visibility
├─→ On-site reading: wind speed, water temp if borderline
│
...
- Air temp below 60°F (we check water temp and dress code)
...
This log lets us defend the call if a guest later complains, and lets us track patterns (e.g., we keep cancelling Honey Island in summer afternoons; should we move the tour earlier?).

---

## Acknowledgement

Knowing this flowchart is required for solo sign-off and is tested in the Comprehension Quiz.
