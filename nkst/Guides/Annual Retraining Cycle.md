...
The cycle aligns with LDWF permit renewal (Dec 31 expiration) and the natural calendar break. Most operations are slow January, which is the right time to do this.
...
| Feb-Mar | Pre-season van inspection (Vehicle Handbook 2.2) |
| Mar | Refresher training session in person if practical (1–2 hours, scenario walk-throughs) |
| Year-round | Mid-cycle re-acknowledgements when policies change materially |
...
Every guide re-reads the full document set:
...
Re-take the NKST Guide Comprehension Quiz. Must pass 80%. Same form as initial onboarding (questions may be tweaked year-over-year as scenarios evolve).

### 3. Annual Acknowledgement Form

Sign the new year's acknowledgement covering all the above plus the ICA companion docs.

### 4. Compliance Refreshes
...
- Personal goals (more tours, different tour types, mentoring)
...
## Mid-Cycle Re-Acknowledgement Triggers

Some changes are too important to wait until next January. Trigger a targeted re-acknowledgement when any of the following happen:

- Major Handbook revision (more than minor edits)
- New SOP introduced
- Anti-Harassment Policy changes
- Vehicle Handbook revisions affecting accident procedure or guide responsibilities
- ICA changes
- New regulatory requirement (LDWF, USCG, parish)

For mid-cycle re-ack: send a one-off Google Form covering only the changed document(s). Track separately, retain in the Guide folder.

---

## Documentation

For each guide, each year:

```
Guides/[Name]/
  acknowledgement_2026.pdf
  quiz_pass_2026-01-15.pdf
  coaching_notes_2026.md
  ldwf_permit_2026.pdf
  cpr_cert_[expires_2027].pdf
  insurance_coi_2026.pdf
  mvr_check_2026-01.pdf (encrypted local; not in cloud)
  notes_changes_2026.md (anything material that happened that year)
```

Retention: 7 years per guide. For terminated guides, retain the same 7 years from the date of last work.

---

## Why This Matters

A plaintiff's attorney asks: "When was the last time this guide was trained on weather thresholds?"

Without this cycle: "We did initial training in 2024. We talk about it sometimes."

With this cycle: "January 15, 2026, this guide re-read the Weather Flowchart, took the quiz scoring 28/30, signed the acknowledgement specifically attesting to the weather thresholds, and attended the March 8, 2026 refresher session where we walked through three weather-related scenarios. Here are the documents."

The second answer is what we want every time.
